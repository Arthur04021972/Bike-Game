...
//ajuste o raio colisor (collider) para o ciclista
...
function draw() {
  ...
  if(keyDown("space") && estadoJogo===ENCERRAMENTO) {
    sinoBicicleta.play();
  }
  ...
  if (World.frameCount % 150 == 0) {
   ...
   if(CGRosa.isTouching(ciclistaPrincipal)){
    ...
    //quando o estado do jogo for encerrar...
}else if (estadoJogo === ENCERRAMENTO) {
    fimdeJogo.visible = true;
    text("Aperte A Tecla Espaço Para Reiniciar", 500, 200)
    //Adicione o código para mostrar o texto com instruções de como reiniciar o jogo aqui ("aperte a tecla para cima para reiniciar"), de dê tamanho 20 com textSize() e cor branca com fill()
 ...
 ciclistaPrincipal.addAnimation("SahilRunning",img2CiclistaPrincipal);
  ...
  //escrever uma condição (usando IF) para quando apertar a tecla pra cima.
  //IMPORTANTE: o bloco IF pedido PRECISA ser escrito aqui
  //dentro do bloco IF, usar a função de recomeçar o jogo
...
// crie e defina a função para recomeçar o jogo aqui

/*A função para recomeçar deve:
- definir o estadoJogo para “JOGAR”.
- tornar a animação fimdeJogo invisível.
- mudar a animação do ciclistaPrincipal para a animação "SahilRunning",ciclistaPrincipalImg1
- destruir os sprites do grupo de ciclistas adversários.
- definir a distância para 0.
*/